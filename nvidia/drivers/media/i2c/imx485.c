...
#include "imx485_mode_tbls.h"
#include "framos_sensor_common.h"

#define IMX485_K_FACTOR 1000LL
#define IMX485_M_FACTOR 1000000LL
#define IMX485_G_FACTOR 1000000000LL
#define IMX485_T_FACTOR 1000000000000LL

#define IMX485_MAX_GAIN_DEC 240
#define IMX485_MAX_GAIN_DB  72

#define IMX485_MAX_BLACK_LEVEL_12BIT 0xFFC
#define IMX485_MAX_BLACK_LEVEL_10BIT 0x3FF

#define IMX485_MIN_SHR0_LENGTH 6
#define IMX485_MIN_INTEGRATION_LINES 2

#define IMX485_4_CSI_LANES 4
#define IMX485_TWO_LANE_MODE 2

#define IMX485_INCK 74250000LL

/**
 * list HEAD of private data of all probed sensors on the platform
 */
LIST_HEAD(imx485_sensor_list);
...
static const struct of_device_id imx485_of_match[] = {
	{ .compatible = "framos,imx485",},
	{ },
};
MODULE_DEVICE_TABLE(of, imx485_of_match);


static int imx485_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx485_custom_ctrl_ops = {
	.s_ctrl = imx485_set_custom_ctrls,
};

const char * const imx485_data_rate_menu[] = {
	[IMX485_1782_MBPS] = "1782 Mbps/lane",
	[IMX485_1440_MBPS] = "1440 Mbps/lane",
};

static const char * const imx485_test_pattern_menu[] = {
	[0] = "No pattern",
	[1] = "000h Pattern",
	[2] = "3FF/FFFh Pattern",
	[3] = "155/555h Pattern",
...
static struct v4l2_ctrl_config imx485_custom_ctrl_list[] = {
	{
		.ops = &imx485_custom_ctrl_ops,
	...
	},
	{
		...
		.type = V4L2_CTRL_TYPE_MENU,
		.min = 0,
		.max = ARRAY_SIZE(imx485_test_pattern_menu) - 1,
		.def = 0,
		.qmenu = imx485_test_pattern_menu,
	},
};
...
static bool imx485_is_binning_mode(struct camera_common_data *s_data)
{
	switch (s_data->mode) {
	case IMX485_MODE_H2V2_BINNING:
		return true;
	default:
		return false;
	}
...
static inline int imx485_read_reg(struct camera_common_data *s_data,
				u16 addr, u8 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
static int imx485_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
 ...
 */
static int imx485_write_reg_broadcast(struct camera_common_data *s_data,
										u16 addr, u8 val)
{
	int err;
	struct device *dev = s_data->dev;
 ...
 */
static int imx485_read_buffered_reg(struct camera_common_data *s_data,
								 u16 addr_low, u8 number_of_registers, u64 *val)
{
	...
	*val = 0;
	...
	if (!s_data->group_hold_active){
		err = imx485_write_reg(s_data, REGHOLD, 0x01);
		if (err) {
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx485_read_reg(s_data, addr_low + i, &reg);
		*val += reg << (i * 8);
		if (err) {
	...
	if (!s_data->group_hold_active){
		err = imx485_write_reg(s_data, REGHOLD, 0x00);
		if (err) {
 ...
 */
static int imx485_write_buffered_reg(struct camera_common_data *s_data,
								u16 addr_low, u8 number_of_registers, u64 val)
{
	int err, i;
	struct device *dev = s_data->dev;
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx485_write_reg(s_data, addr_low + i, (u8)(val >> (i * 8)));
		if (err) {
 ...
 */
static int imx485_broadcast_buffered_reg(struct camera_common_data *s_data,
								u16 addr_low, u8 number_of_registers, u32 val)
{
	...
	if (!s_data->group_hold_active){
		err = imx485_write_reg_broadcast(s_data, REGHOLD, 0x01);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
			return err;
		}
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx485_write_reg_broadcast(s_data, addr_low + i,
					(u8)(val >> (i * 8)));
		if (err) {
			dev_err(dev, "%s: BUFFERED register write error\n", __func__);
			return err;
	...
	if (!s_data->group_hold_active){
		err = imx485_write_reg_broadcast(s_data, REGHOLD, 0x00);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD erroror\n", __func__);
			return err;
		}
...
static int imx485_write_table(struct imx485 *priv,
				const imx485_reg table[])
{
	...
	return regmap_util_write_table_8(s_data->regmap,
					 table,
					 NULL, 0,
					 IMX485_TABLE_WAIT_MS,
					 IMX485_TABLE_END);
}

static int imx485_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	...
	err = imx485_write_reg(s_data, REGHOLD, val);
	if (err) {
		dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
		return err;
	}
...
static int imx485_update_ctrl(struct tegracam_device *tc_dev, u64 val)
{
...
static int imx485_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	switch (s_data->colorfmt->code) {
		...
		default:
			if (val > IMX485_MAX_BLACK_LEVEL_10BIT)
				val = IMX485_MAX_BLACK_LEVEL_10BIT;
			err = imx485_update_ctrl(tc_dev, val);
			black_level_reg = val;
	}

	err = imx485_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, black_level_reg);
	if (err){
...
static int imx485_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	...
	/* translate value */
	gain = val * IMX485_MAX_GAIN_DEC /
				 (IMX485_MAX_GAIN_DB *
					 mode->control_properties.gain_factor);


	if (priv->broadcast_ctrl == BROADCAST)
		err = imx485_broadcast_buffered_reg(s_data,
											 GAIN_LOW, 2, gain);
	else {
		err = imx485_write_buffered_reg(s_data,
											GAIN_LOW, 2, gain);
	}
	if (err){
...
static int imx485_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	integration_time_line = (val * IMX485_K_FACTOR) / priv->line_time ;
	...
	if (reg_shr0 < IMX485_MIN_SHR0_LENGTH)
		reg_shr0 = IMX485_MIN_SHR0_LENGTH;
	else if (reg_shr0 > (priv->frame_length - IMX485_MIN_INTEGRATION_LINES))
		reg_shr0 = priv->frame_length - IMX485_MIN_INTEGRATION_LINES;

	if (priv->broadcast_ctrl == BROADCAST)
		err = imx485_broadcast_buffered_reg(s_data, SHR0_LOW, 3, reg_shr0);
	else
		err = imx485_write_buffered_reg (s_data, SHR0_LOW, 3, reg_shr0);
	if (err) {
...
static int imx485_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	...
	frame_length = (((u64)mode->control_properties.framerate_factor *
							  IMX485_G_FACTOR) / (val * priv->line_time));
	...
	exposure_min_range = IMX485_MIN_INTEGRATION_LINES
											* priv->line_time / IMX485_K_FACTOR;

	exposure_max_range = (priv->frame_length - IMX485_MIN_INTEGRATION_LINES)
											* priv->line_time / IMX485_K_FACTOR;
 ...
 * in the IMX485 support package
 */
static int imx485_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	if (val) {
		err = imx485_write_table(priv, mode_table[IMX485_EN_PATTERN_GEN]);
		if (err)
			goto fail;
		err = imx485_write_reg(s_data, TPG_PATSEL_DUOUT, (u8)(val - 1));
		if (err)
			goto fail;
	} else {
 ...
 */
static int imx485_update_framerate_range(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	...
	if (imx485_is_binning_mode(s_data))
		priv->min_frame_length =  IMX485_DEFAULT_HEIGHT
										+ IMX485_MIN_FRAME_LENGTH_DELTA;
	else
		priv->min_frame_length = s_data->fmt_height
										+ IMX485_MIN_FRAME_LENGTH_DELTA;

	max_framerate = (IMX485_G_FACTOR * IMX485_M_FACTOR) /
 ...
 * Set streaming mode
 */
static int imx485_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
 ...
 */
static int imx485_set_operation_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
...
static int imx485_set_sync_feature(struct tegracam_device *tc_dev, u32 val)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
...
static int imx485_set_broadcast_ctrl(struct tegracam_device *tc_dev,
										struct v4l2_ctrl *ctrl)
{
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	imx485_configure_second_slave_address();
...
static struct tegracam_ctrl_ops imx485_ctrl_ops = {
	.numctrls = ARRAY_SIZE(ctrl_cid_list),
	.ctrl_cid_list = ctrl_cid_list,
	.set_gain = imx485_set_gain,
	.set_exposure = imx485_set_exposure,
	.set_frame_rate = imx485_set_frame_rate,
	.set_group_hold = imx485_set_group_hold,
	.set_test_pattern = imx485_set_test_pattern,
	.set_streaming_mode = imx485_set_streaming_mode,
	.set_operation_mode = imx485_set_operation_mode,
	.set_sync_feature = imx485_set_sync_feature,
	.set_broadcast_ctrl = imx485_set_broadcast_ctrl,
	.set_black_level = imx485_set_black_level,
};

static int imx485_power_on(struct camera_common_data *s_data)
{
	int err = 0;
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
	struct imx485 *priv = (struct imx485 *)s_data->priv;
	...
	if (!pw->mclk) {
		...
		goto imx485_mclk_fail;
	}

	/* Power ON sequence according to IMX485 datasheet */

	if (pw->dvdd) {
		err = regulator_enable(pw->dvdd);
		if (err)
			goto imx485_dvdd_fail;
	}

	if (pw->iovdd) {
		err = regulator_enable(pw->iovdd);
		if (err)
			goto imx485_iovdd_fail;
	}

	if (pw->avdd) {
		err = regulator_enable(pw->avdd);
		if (err)
			goto imx485_avdd_fail;
	}
...
static int imx485_power_off(struct camera_common_data *s_data)
{
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
	 ...
	 * in the chapter "XVS/XHS IO setting" in the IMX485 Synchronizing
	 * Sensors App Note
	 */
	err = imx485_write_reg(s_data, XVS_XHS_DRV, 0xF);
	if (err) {
	...
	/* Power OFF sequence according to IMX485 datasheet */
 ...
 */
static int imx485_power_get(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_power_rail *pw = s_data->power;
 ...
 */
static int imx485_power_put(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
 ...
 */
static int imx485_communication_verify(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	err = imx485_read_buffered_reg(s_data, VMAX_LOW, 2, &vmax);
	if (err) {
	...
	err = imx485_read_buffered_reg(s_data, BLKLEVEL_LOW, 2, &black_level);
	if (err) {
...
static struct camera_common_pdata *imx485_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	...
	match = of_match_device(imx485_of_match, dev);
	if (!match) {
...
static int imx485_set_pixel_format(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	...
	case MEDIA_BUS_FMT_SRGGB10_1X10:
		err = imx485_write_table(priv, mode_table[IMX485_10BIT_MODE]);
		break;
	case MEDIA_BUS_FMT_SRGGB12_1X12:
		err = imx485_write_table(priv, mode_table[IMX485_12BIT_MODE]);
		break;
	default:
 ...
 */
static int imx485_set_csi_lane_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	if (s_data->numlanes == IMX485_TWO_LANE_MODE) {
		err = imx485_write_reg(s_data, LANEMODE, 1);
		if (err) {
 ...
 * Calculate 1H time
 */
static int imx485_calculate_line_time(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx485_read_buffered_reg(s_data, HMAX_LOW, 2, &hmax);
	if (err) {
		dev_err(dev, "%s: unable to read hmax\n", __func__);
		return err;
	}

	priv->line_time = (hmax*IMX485_G_FACTOR) / (IMX485_INCK);
 ...
 */
static int imx485_adjust_hmax_register(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	int err;
	u64 hmax = 0x226;
	u8 csi_lane_coef = IMX485_4_CSI_LANES / s_data->numlanes;

	dev_dbg(dev, "%s:++\n", __func__);

	switch (priv->data_rate) {
	case IMX485_1782_MBPS:
		if (s_data->numlanes == 2)
			if (imx485_is_binning_mode(s_data)) {
				hmax = 0x10C * csi_lane_coef;
			}
			else if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10) {
				hmax = 0x1B0 * csi_lane_coef;
			} else {
				hmax = 0x200 * csi_lane_coef;
			}
		else {
	...
	err = imx485_write_buffered_reg(s_data, HMAX_LOW, 2, hmax);
	if (err) {
 ...
 * in the IMX485 datasheet
 */
static int imx485_verify_data_rate(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	/* Binning mode */
	if (imx485_is_binning_mode(s_data) ) {
		/* Bining mode 4 CSI lanes */
		if (s_data->numlanes != IMX485_TWO_LANE_MODE
			&& priv->data_rate == IMX485_1782_MBPS) {
			priv->data_rate = IMX485_1440_MBPS;
			goto modify_ctrl;
		}
	}
...
/**
 * IMX485 allows for multiple data rates. Available data rates are described
 * in the chapter "Readout Drive mode" in the IMX485 datasheet
 */
static int imx485_set_data_rate(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx485_verify_data_rate(tc_dev);
	if (err)
		goto fail;

	err = imx485_write_table(priv, data_rate_table[priv->data_rate]);
	if (err)
		goto fail;
 ...
 */
static int imx485_set_sync_mode(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx485_write_reg(s_data, EXTMODE, extmode);
	if (err)
 ...
 */
static bool imx485_find_broadcast_sensor(struct imx485 *broadcast_private)
{
	struct imx485 *current_private;

	list_for_each_entry(current_private, &imx485_sensor_list, entry) {
 ...
 */
static void imx485_enable_second_slave_address(struct imx485 *ack_private)
{
	struct imx485 *current_private;
	int err;
	...
	list_for_each_entry(current_private, &imx485_sensor_list, entry) {
		...
		err = imx485_write_reg(current_private->s_data,
								SECOND_SLAVE_ADD, 1);
		if (err)
	...
	err = imx485_write_reg(ack_private->s_data,
								SECOND_SLAVE_ADD, 3);
	if (err)
 ...
 */
static void imx485_disable_second_slave_address(void)
{
	struct imx485 *current_private;
	int err;
...
static void imx485_configure_second_slave_address(void)
{
	struct imx485 broadcast_private = {};

	if (imx485_find_broadcast_sensor(&broadcast_private)) {
		imx485_enable_second_slave_address(&broadcast_private);
	} else {
		imx485_disable_second_slave_address();
	}
}
 ...
 * 0xF - hi-z,   hi-z
 */
static int imx485_configure_triggering_pins(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx485_write_reg(s_data, XVS_XHS_DRV, xvs_xhs_drv);
	if (err) {
 ...
 */
static bool imx485_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct imx485 *current_priv;
	struct device *dev = tc_dev->dev;
	u8 streaming_mode = priv->current_streaming_mode;

	list_for_each_entry(current_priv, &imx485_sensor_list, entry) {
 ...
 */
static bool imx485_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tc_dev->priv;
	struct imx485 *current_priv;
	struct device *dev = tc_dev->dev;
 ...
 */
int imx485_check_unsupported_mode(struct camera_common_data *s_data,
...
static int imx485_set_mode(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx485_set_csi_lane_mode(tc_dev);
	if (err) {
		dev_err(dev, "%s: error setting CSI lane mode\n", __func__);
		return err;
	}

	err = imx485_set_pixel_format(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to write format to image sensor\n", __func__);
		return err;
	}

	err = imx485_write_table(priv, mode_table[s_data->mode]);
	if (err)
		return err;

	err = imx485_set_operation_mode(tc_dev, priv->current_operation_mode);
	if (err) {
		dev_err(dev, "%s: unable to operation mode\n", __func__);
		return err;
	}

	err = imx485_set_sync_mode(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to set sync mode\n", __func__);
		return err;
	}

	err = imx485_configure_triggering_pins(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable configure XVS/XHS pins\n", __func__);
		return err;
	}

	err = imx485_set_data_rate(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to set data rate\n", __func__);
		return err;
	}

	err = imx485_adjust_hmax_register(tc_dev);
	if (err) {
	...
	if (!imx485_is_stream_configuration_valid(tc_dev)) {
		dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
		return -EPERM;
	}

	if (!imx485_is_pin_configuration_valid(tc_dev)) {
		dev_err(dev, "%s: illegal pin configuration detected\n", __func__);
		return -EPERM;
	}

	err = imx485_calculate_line_time(tc_dev);
	if (err)
		return err;
...
static int imx485_stop_streaming(struct tegracam_device *tc_dev)
{
	struct imx485 *priv = (struct imx485 *)tegracam_get_privdata(tc_dev);
	int err;
	 ...
	 */
	usleep_range(priv->frame_length * priv->line_time / IMX485_K_FACTOR,
				priv->frame_length * priv->line_time / IMX485_K_FACTOR + 1000);

	return 0;
}


static struct camera_common_sensor_ops imx485_common_ops = {
	.numfrmfmts = ARRAY_SIZE(imx485_frmfmt),
	.frmfmt_table = imx485_frmfmt,
	.power_on = imx485_power_on,
	.power_off = imx485_power_off,
	.write_reg = imx485_write_reg,
	.read_reg = imx485_read_reg,
	.parse_dt = imx485_parse_dt,
	.power_get = imx485_power_get,
	.power_put = imx485_power_put,
	.set_mode = imx485_set_mode,
	.start_streaming = imx485_start_streaming,
	.stop_streaming = imx485_stop_streaming,
	.check_unsupported_mode = imx485_check_unsupported_mode,
};


static int imx485_board_setup(struct imx485 *priv)
{
	struct camera_common_data *s_data = priv->s_data;
	struct device *dev = s_data->dev;
	int err = 0;
	...
	err = imx485_communication_verify(priv->tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to communicate with sensor\n",  __func__);
		goto error;
	}

	err = imx485_calculate_line_time(priv->tc_dev);
	if (err) {
...
static int imx485_ctrls_init(struct tegracam_device *tc_dev)
{
	...
	numctrls = ARRAY_SIZE(imx485_custom_ctrl_list);

	for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx485_custom_ctrl_list[i];
			...
			const struct i2c_device_id *id)
{
	...
	priv = devm_kzalloc(dev,
			sizeof(struct imx485), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	tc_dev->dev = dev;
	strncpy(tc_dev->name, "imx485", sizeof(tc_dev->name));
	tc_dev->dev_regmap_config = &sensor_regmap_config;
	tc_dev->sensor_ops = &imx485_common_ops;
	tc_dev->v4l2sd_internal_ops = &imx485_subdev_internal_ops;
	tc_dev->tcctrl_ops = &imx485_ctrl_ops;
	...
	priv->s_data->blklvl_max_range = IMX485_MAX_BLACK_LEVEL_12BIT;
	...
	dev_info(dev, "Detected imx485 sensor\n");

	return 0;
}

static int imx485_remove(struct i2c_client *client)
{
...
static const struct i2c_device_id imx485_id[] = {
	{ "imx485", 0 },
	{ }
};

MODULE_DEVICE_TABLE(i2c, imx485_id);

static struct i2c_driver imx485_i2c_driver = {
	.driver = {
		.name = "imx485",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(imx485_of_match),
	},
	.probe = imx485_probe,
	.remove = imx485_remove,
	.id_table = imx485_id,
};

module_i2c_driver(imx485_i2c_driver);

MODULE_DESCRIPTION("Media Controller driver for Sony IMX485");
