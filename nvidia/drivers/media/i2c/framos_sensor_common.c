...
int supported_fw_version[4] = {1, 2, 0, 0};
int supported_latest_fw_version[4] = {1, 2, 1, 0};

void fm_get_board_model(struct tegracam_device *tc_dev, u8 *som_board_model)
{
	...
	if (of_property_read_string(root, "model", &model)) {
		dev_err(dev, "Cannot read board model\n");
    }
	of_node_put(root);

    if(!strcmp(model, "NVIDIA Jetson Nano Developer Kit")){
        *som_board_model = JETSON_NANO_DEV_KIT;
    }
    else if(!strcmp(model, "quill-3310")){
        *som_board_model = JETSON_TX2;
    }
    else if(!strcmp(model, "Jetson-AGX")){
        *som_board_model = JETSON_XAVIER;
    }
...
EXPORT_SYMBOL(fm_get_board_model);
                                            ...
                                            s32 *val)
{
    ...
    dev_dbg(dev, "%s: CS fw ver.:%d.%d.%d.%d\n", 
                    __func__, fw_version[0], fw_version[1], fw_version[2], fw_version[3]);

    for (i = 0; i<4; i++){
        if (fw_version[i] == supported_fw_version[i]){
            cnt1 += 1;
        }
        if (fw_version[i] == supported_latest_fw_version[i]){
            cnt2 += 1;
        }
    }
    ...
    if (fw_ver_err){
        dev_err(dev, "Image sensor driver don't support this firmware version. Most features will probably not work.\n");
        dev_err(dev, "RECONFIGURE FIRMWARE with the latest version - %d.%d.%d.%d!\n", 
            supported_latest_fw_version[0], supported_latest_fw_version[1], supported_latest_fw_version[2], supported_latest_fw_version[3]);
    } 

    else if (fw_ver_warn){
        dev_err(dev, "Crosslink is not configured with the latest firmware version (Latest version - %d.%d.%d.%d). Some features may not work properly\n", 
            supported_latest_fw_version[0], supported_latest_fw_version[1], supported_latest_fw_version[2], supported_latest_fw_version[3]);
    } 

    err = check_crosslink_image_data(tc_dev, image_data);
    if (err) {
        dev_err(dev, "Crosslink firmware and sensor configuration doesn't match!\n");
        ...
        if (fw_ver_err){
            dev_err(dev, "RECONFIGURE FIRMWARE with %d.%d.%d.%d version!\n", 
            supported_latest_fw_version[0], supported_latest_fw_version[1], supported_latest_fw_version[2], supported_latest_fw_version[3]);
        }
        else
        {
